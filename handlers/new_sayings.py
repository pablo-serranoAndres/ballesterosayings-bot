...
from ui.markup import confirm_new_saying, general_menu, go_home_indicator
...
def handle_cb_new_saying(session: User, bot:TeleBot, chat_id:int, aditional_params:str):
    ...
    bot_message(bot, chat_id, session.user_id, AppAction.NEW_SAYING, go_home_indicator(session.user_id))

    session.menu_status = AppAction.WAITING_TITLE
...
def handle_cb_save_new_saying(session: User, bot:TeleBot, chat_id:int, aditional_params:str):
    ...
    session.menu_status = AppAction.DATA_SAVED
    
def handle_mss_new_saying(message, bot:TeleBot, session:User):
    chat_id = message.chat.id
    message_text = message.text
    
    if (session.menu_status == AppAction.WAITING_TITLE): 
        ...
        session.menu_status = AppAction.WAITING_DESCRIPTION
    
    elif (session.menu_status == AppAction.WAITING_DESCRIPTION):
        new_saying[session.user_id].description = message_text
        bot_message(bot, chat_id, session.user_id, session.menu_status, go_home_indicator(session.user_id))

        session.menu_status =  AppAction.WAITING_AUTHOR
        
    elif (session.menu_status == AppAction.WAITING_AUTHOR): 
        new_saying[session.user_id].author = message_text
        session.menu_status =  AppAction.CONFIRM_CREATION
