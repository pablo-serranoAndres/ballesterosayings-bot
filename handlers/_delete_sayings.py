...
from ui.keyboard_factory import general_menu, go_home_indicator, saying_item_delete
...
def handle_cb_delete_saying(session:User, bot:TeleBot, chat_id:int, aditional_params:str):
    bot_message(bot, chat_id,session.user_id, AppAction.ASK_ID_DELETE, go_home_indicator(session.user_id)) 
    session.menu_status = AppAction.SEND_SAYING_DELETE
...
def handle_cb_keep_saying(session:User, bot:TeleBot, chat_id:int, aditional_params:str):
    bot_message(bot, chat_id, session.user_id, AppAction.KEEP_SAYING, general_menu(session.user_id))
    session.menu_status = AppAction.INTRODUCTION
...
def handle_mss_delete_saying (message, bot:TeleBot, session:User) : 
    ...
    if (session.menu_status == AppAction.ASK_ID_DELETE) : 
        bot_message(bot, chat_id,session.user_id, session.menu_status, go_home_indicator(session.user_id))

        return AppAction.SEND_SAYING_DELETE

    elif (session.menu_status == AppAction.SEND_SAYING_DELETE):
        ...
        if (saying_to_eliminate[session.user_id]):
            bot_message(bot, chat_id, session.user_id, session.menu_status, saying_item_delete(session.user_id), saying_to_eliminate[session.user_id])
             ...
             bot_message(bot, chat_id, session.user_id, AppAction.NO_DATA_FOUND ,go_home_indicator(session.user_id))
