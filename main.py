...
from handlers.admin import handle_new_user, send_auth_to_admin
...
def initialize_bot(message):
    ...
    if (session.autorized == False):
        handle_new_user(session, bot, chat_id, f'reject-{user_id}')      
        return
...
def callback_query(call):
    ...
    if session.autorized == True:
        ...
        handle_new_user(session, bot, call.message.chat.id, f'reject-{user_id}')
